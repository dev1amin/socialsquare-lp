...
export function BlueWebGLShader() {
  ...
  useEffect(() => {
    ...
    // Blue-tinted shader matching SocialSquare #4167B2
    const fragmentShader = `
      ...
      void main() {
        ...
        // Single sine wave — no chromatic aberration
        float wave = 0.05 / abs(p.y + sin((p.x + time) * xScale) * yScale);

        // SocialSquare blue: #4167B2
        vec3 blue = vec3(0.255, 0.404, 0.698);
        vec3 col = blue * wave;
        float brightness = wave;

        // Transparent background — only the blue line is visible
        gl_FragColor = vec4(col, smoothstep(0.02, 0.12, brightness));
      }
    `;
